...
function cevhershare($print = true){
    ...
    $str = '<ul class="cevhershare" style="'.$seffaf.'">';
	$results = $wpdb->get_results("SELECT * FROM "."wp_1_"."cevhershare WHERE enabled=1 ORDER BY position, id ASC"); $str .= "\n";
...
function cevhershare_button($name, $size = 'big'){
	global $wpdb;
	$item = $wpdb->get_row("SELECT * FROM "."wp_1_"."cevhershare WHERE name='$name'");
...
function cevhershare_update_button($id, $uptask){
	global $wpdb;
	if($uptask == 'enable')
		$wpdb->query("UPDATE "."wp_1_"."cevhershare SET enabled='1' WHERE id='$id'");
	elseif($uptask == 'disable')
		$wpdb->query("UPDATE "."wp_1_"."cevhershare SET enabled='0' WHERE id='$id'");
	elseif($uptask == 'delete')
		$wpdb->query("DELETE FROM "."wp_1_"."cevhershare WHERE id=$id LIMIT 1");
}
