...
// TODO: log the location it came from
  ...
  if (!empty($giftID)) {
    if (substr($giftID, 0,1) == 'x') {
      ...
      cart_add($cart->id, $giftID, $qty, $abs, $_REQUEST['amount'],null,null,null,null, $eid, $bid);
...
if (isset($_POST['is_cart'])) {
  ...
  $test = ($_POST['test_payment'] == "on");
  if (can_make_test_payment()) {
    // cart_test($cart->id, $test);
    // dc($cart->id, 'Cart is set to ' . ($test ? "TEST" : "real"));
  }
...
if ($eid > 0) {
  // Is the cart empty?
  if( count_cart_items($cart->id) == 0 && $cart->status == 'active' ) {
    $url = get_campaign_permalink($cart->data->event_id);
    wp_redirect(add_query_arg("m","cart_empty", $url));
    die;
  }

/*
  $cart->campaign = get_campaign_stats($eid);
...
if ($is_empty) {
  ...
  ?>
  <form id="cart-page" action="<?= remove_query_arg('item') ?>" method="post" class="standard-form cart-page page-content based no-enter evs">
...
////////////////////////////////////////////////////////////////////////////////
  ...
  display_test_mode_switch($cart->id);
