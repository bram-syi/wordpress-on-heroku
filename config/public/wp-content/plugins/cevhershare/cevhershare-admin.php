<?php
	...
	*/
	...
	if($id)	$item = $wpdb->get_row("SELECT * FROM "."wp_1_"."cevhershare WHERE id=$id");
	if($do == 'update') $wpdb->query("UPDATE "."wp_1_"."cevhershare SET enabled='".$_POST['enabled']."', position='".$_POST['position']."', name='".$_POST['name']."', big='".$_POST['big']."', small='".$_POST['small']."' WHERE id='$id'");
	elseif($do == 'add') $wpdb->query("INSERT INTO "."wp_1_"."cevhershare (position, name, big, small) VALUES('".$_POST['position']."','".$_POST['name']."', '".$_POST['big']."', '".$_POST['small']."')");
	elseif($do == 'delete') $wpdb->query("DELETE FROM "."wp_1_"."cevhershare WHERE id=$id LIMIT 1");
	...
	elseif($dil == 'turkish'){
	include_once ('lang/lang_tr.php');}	//dil bitdi	
	if($pos == 'moveup') $wpdb->query("UPDATE "."wp_1_"."cevhershare SET position=position-1 WHERE id='$id'");
	if($pos == 'movedown') $wpdb->query("UPDATE "."wp_1_"."cevhershare SET position=position+1 WHERE id='$id'");
...
<?php }else{ ?>
	...
	<table id="cevhershare-tl">
		...
		<tbody>	
		<?php $results = $wpdb->get_results("SELECT * FROM "."wp_1_"."cevhershare ORDER BY position, id ASC"); echo "\n";
