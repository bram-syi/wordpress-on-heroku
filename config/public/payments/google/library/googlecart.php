 ...
 */
  ...
  */
  class GoogleCart {
     ...
     *                         defaults to 'USD'
		 */
    function GoogleCart($id, $key, $server_type="sandbox", $currency="USD", $server_url="https://sandbox.google.com/checkout/") {
      $this->merchant_id = $id;
...
//      New Digital Delivery Tags
        if($item->digital_content) {
          ...
          if(!empty($item->digital_url) || !empty($item->digital_description)) {
...
//            To avoid NULL key message in GC confirmation Page
          ...
          $xml_data->element('display-disposition','OPTIMISTIC');
